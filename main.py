	...
	try:
		with sqlite3.connect(DB_PATH) as connection:
			with closing(connection.cursor()) as cursor:
				cursor.execute("SELECT UPPER([id]) AS id, UPPER([descripción]) AS nm FROM [Ciclo Curricular] WHERE ([estados] & 1)=1 ORDER BY [ciclo numérico]", ())
...
@app.route('/CICLO_CURRICULAR/v1/_/', method='GET')
def get_courses():
	try:
		with sqlite3.connect(DB_PATH) as connection:
			with closing(connection.cursor()) as cursor:
				cursor.execute("SELECT UPPER([id]) AS id, UPPER([descripción]) AS nm, (([estados] & 1) = 1) AS st FROM [Ciclo Curricular] ORDER BY [ciclo numérico]", ())
				cursor.row_factory = sqlite3.Row
				ds_ = [dict(r) for r in cursor.fetchall()]
				return bottle.HTTPResponse(body=json.dumps({'data': ds_}), status=200, headers=MINIMAL_CORS)
	except sqlite3.OperationalError as e:
